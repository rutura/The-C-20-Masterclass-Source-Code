...
template<typename T, typename K> void print_map(const std::map<T, K> &map)
{
...
int main()
{
  ...
  for (const auto &[key, value] : books) { fmt::println("  book [{}]: {}", key, value); }

  fmt::println("");
  fmt::println(" Traverse map with C++11 loop syntax : ");
  ;
  for (const auto &elt : books) { fmt::println("  book [{}]:{}", elt.first, elt.second); }

  fmt::println("");
  fmt::println(" traverse map with loop (with explicit types) : ");
  for (const std::pair<int, Book> &elt : books) { fmt::println("  book [{}, {}]", elt.first, elt.second); }
  ...
  for (const auto &[key, value] : address_book) { fmt::println(" {} lives at address {}", key, value); }
  ...
  */
  ...
  fmt::print("numbers2 : [ ");
  for (const auto &[key, value] : numbers2) { fmt::print(" ({}, {})", key, value); }
  fmt::println("]");
