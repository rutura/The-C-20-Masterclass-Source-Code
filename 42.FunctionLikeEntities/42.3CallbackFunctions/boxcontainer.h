...
// Move constructor
template<typename T> BoxContainer<T>::BoxContainer(BoxContainer &&source)
{
...
template<typename T> void BoxContainer<T>::add(T &&item)
{
  ...
  if (m_size == m_capacity) expand(m_size + EXPAND_STEPS);
  // m_items[m_size] = item;
...
template<typename T> void BoxContainer<T>::operator=(BoxContainer &&source)
{
