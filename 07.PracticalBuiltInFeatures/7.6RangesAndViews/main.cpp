...
int main() {
    ...
    show_values(counted, "Generate integers 1-10");
    ...
    auto from_array{
        numbers | std::views::filter([](int x) { return x % 2 == 0; })
                | std::views::transform([](int x) { return x * x; })};
    show_values(from_array, "Squares of even integers in numbers");
