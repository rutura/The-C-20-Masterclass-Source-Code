...
int main() {
...

    {
        ...
        std::chrono::time_point<std::chrono::steady_clock, std::chrono::seconds> tp_seconds{42s};
        std::chrono::time_point<std::chrono::steady_clock, std::chrono::milliseconds>
            tp_milliseconds{tp_seconds};

        std::println("{}", tp_milliseconds.time_since_epoch());   // 42000ms
    }

    {
        ...
        std::chrono::time_point<std::chrono::steady_clock, std::chrono::milliseconds>
            tp_milliseconds{42'424ms};

        std::chrono::time_point<std::chrono::steady_clock, std::chrono::seconds> tp_seconds{
            std::chrono::time_point_cast<std::chrono::seconds>(tp_milliseconds)};
        // Or, letting the compiler deduce the type:
        //   auto tp_seconds{std::chrono::time_point_cast<std::chrono::seconds>(tp_milliseconds)};

        // Converting back to milliseconds shows the lost precision: 42424ms
        // became 42000ms, not the original value.
        std::chrono::milliseconds ms{tp_seconds.time_since_epoch()};
