...
namespace templates_and_friendship_08 {

// BoxContainer with a stream insertion operator

// Forward declare friends : These forward declarations are needed for
// successful compilation on all major compilers : GCC,Clang,MSVC. template
// <typename T> class BoxContainer; template <typename T> std::ostream&
// operator<<( std::ostream&, const BoxContainer<T>& );
...
template<typename T> class BoxContainer
{
  ...
  // Either of these solutions will need the friend function template
  // predeclared. More info in the shared isocpp link.
  friend std::ostream &operator<<<>(std::ostream &, const BoxContainer<T> &);
...
public:
  ...
  /*
  friend std::ostream& operator<<(std::ostream& out, const BoxContainer<T>&
  operand)
  {
...
/*
 template < typename T>
inline std::ostream& operator<<(std::ostream& out, const BoxContainer<T>&
operand){
...
template<typename T> BoxContainer<T>::~BoxContainer() { delete[] m_items; }
...
template<typename T> void BoxContainer<T>::add(const T &item)
{
  if (m_size == m_capacity)
    // expand(m_size+5); // Let's expand in increments of 5 to optimize on the
    // calls to expand
...
template<typename T> bool BoxContainer<T>::remove_item(const T &item)
{
  ...
  size_t index{ m_capacity + 999 };// A large value outside the range of the
  // current array
