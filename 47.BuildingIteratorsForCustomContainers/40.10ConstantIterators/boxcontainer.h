...
template<typename T> requires std::is_default_constructible_v<T> class BoxContainer
{
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out, const BoxContainer<T> &operand)
  {
    ...
    for (size_t i{ 0 }; i < operand.m_size; ++i) { out << operand.m_items[i] << " "; }
    out << "]";
...
public:
  ...
  public:
    ...
    difference_type operator-(const Iterator &right) const { return m_ptr - right.m_ptr; }
  ...
  public:
    ...
    ConstIterator &operator-=(const difference_type offset) { return *this += -offset; }
    ...
    difference_type operator-(const ConstIterator &right) const { return m_ptr - right.m_ptr; }

    reference_type operator[](const difference_type offset) const { return *(*this + offset); }

    bool operator<(const ConstIterator &right) const { return m_ptr < right.m_ptr; }

    bool operator>(const ConstIterator &right) const { return right < *this; }
...
// Free operators
template<typename T>
requires std::is_default_constructible_v<T> BoxContainer<T> operator+(const BoxContainer<T> &left,
  const BoxContainer<T> &right);
template<typename T> void format_int_pointer_array(fmt::basic_memory_buffer<char> &out, T const *array, size_t size)
{
  format_to_n(std::back_inserter(out), 1, "[");
  for (size_t i = 0; i < size; ++i) {
    format_to_n(std::back_inserter(out), 1, "{}", (array[i]));
    if (i < size - 1) { format_to_n(std::back_inserter(out), 1, ", "); }
  }
  format_to_n(std::back_inserter(out), 1, "]");
}
template<typename T> struct fmt::formatter<BoxContainer<T>>
{
  constexpr auto parse(format_parse_context &ctx) { return ctx.begin(); }
  template<typename FormatContext> auto format(const BoxContainer<T> &obj, FormatContext &ctx)
  {
    memory_buffer buf;
    format_int_pointer_array(buf, obj.m_items, obj.size());
    return format_to(
      ctx.out(), "BoxContainer: [size: {}, capacity: {}, items: {}]", obj.size(), obj.capacity(), fmt::to_string(buf));
  }
};
...
template<typename T> requires std::is_default_constructible_v<T> BoxContainer<T>::BoxContainer(size_t capacity)
{
...
template<typename T>
requires std::is_default_constructible_v<T> BoxContainer<T>::BoxContainer(
  const BoxContainer<T> &source) requires std::copyable<T>
{
  // Set up the new box
  ...
  for (size_t i{}; i < source.size(); ++i) { m_items[i] = source.m_items[i]; }
}

template<typename T> requires std::is_default_constructible_v<T> BoxContainer<T>::~BoxContainer() { delete[] m_items; }


template<typename T> requires std::is_default_constructible_v<T> void BoxContainer<T>::expand(size_t new_capacity)
{
  ...
  for (size_t i{}; i < m_size; ++i) { new_items_container[i] = m_items[i]; }
...
template<typename T> requires std::is_default_constructible_v<T> void BoxContainer<T>::add(const T &item)
{
...
template<typename T> requires std::is_default_constructible_v<T> bool BoxContainer<T>::remove_item(const T &item)
{
...
template<typename T> requires std::is_default_constructible_v<T> size_t BoxContainer<T>::remove_all(const T &item)
{
...
template<typename T>
requires std::is_default_constructible_v<T> void BoxContainer<T>::operator+=(const BoxContainer<T> &operand)
{
  ...
  for (size_t i{}; i < operand.m_size; ++i) { m_items[m_size + i] = operand.m_items[i]; }
...
template<typename T>
requires std::is_default_constructible_v<T> void BoxContainer<T>::operator=(const BoxContainer<T> &source)
{
  T *new_items;
