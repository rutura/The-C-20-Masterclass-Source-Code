...
struct fmt::formatter<CivilEngineer> {
    ...
    auto format(const CivilEngineer &obj, FormatContext &ctx) const {
        return format_to(ctx.out(), "CivilEngineer [ Full name: {}, age: {}, contract_count: {}, specialty: {}]", obj.get_full_name(), obj.get_age(), obj.get_address(), obj.get_contract_count(), obj.m_speciality);
    }
};
