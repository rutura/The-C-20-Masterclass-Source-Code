...
// std::print/std::format need a formatter specialization - this one just
// reuses the operator<< above via an ostringstream, so both interop paths
// share one implementation.
template <typename T>
struct std::formatter<BoxContainer<T>> : std::formatter<std::string> {
	auto format(const BoxContainer<T>& box, std::format_context& ctx) const {
