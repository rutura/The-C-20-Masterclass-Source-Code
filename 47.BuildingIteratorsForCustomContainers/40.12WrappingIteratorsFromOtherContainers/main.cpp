...
template<typename T> struct fmt::formatter<VectorWrapper<T>>
{
  constexpr auto parse(format_parse_context &ctx) { return ctx.begin(); }
  template<typename FormatContext> auto format(const VectorWrapper<T> &obj, FormatContext &ctx)
  {
    return format_to(ctx.out(),
      "VectorWrapper: [size: {}, capacity: {}, items: {}]",
      obj.m_items.size(),
      obj.m_items.capacity(),
      fmt::join(obj.m_items | std::views::transform([](const T &elem) { return fmt::format("{}", elem); }), " "));
  }
};
...
int main()
{
  ...
  for (auto i : greeting) { fmt::print("{} ", i); }
  fmt::println("");


  fmt::println("taking only 2 : ");
  for (auto i : greeting | std::views::take(2)) { fmt::print("{} ", i); }
  fmt::println("");
