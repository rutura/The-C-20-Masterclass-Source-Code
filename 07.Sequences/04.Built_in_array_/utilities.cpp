...
void size_of_an_array()
{
  ...
  */
  ...
  for (size_t i{ 0 }; i < count; ++i) { fmt::println("scores [{}] : {}", i, scores[i]); }

  //Range based for loop
  for (auto i : scores) { fmt::println("value  : {}", i); }
}
...
void random_num_modern_cplusplus()
{
  ...
  for (size_t count{ 0 }; count < 48; ++count) { fmt::println("{}", die(mersenne)); }
}


void use_random_numbers()
{
