...
void read_write_binary_files()
{
  ...
  } else { fmt::print("No content read from file.\n"); }
}


void read_write_pod_types()
{
  ...
  if (pod_types::write_pod(file_path, pod)) { fmt::print("PODType object written to file.\n"); } else {
  ...
  } else { fmt::print("Failed to read PODType object from file.\n"); }

}

void write_and_read_several_pod_types()
{
  ...
  if (pod_types::write_pod_vector(file_path, pod_vector)) { fmt::print("PODType objects written to file.\n"); } else {
  ...
  if (pod_types::read_pod_vector(file_path, read_pod_vector)) {
    ...
    for (const auto &pod : read_pod_vector) { fmt::print("id={}, value={}\n", pod.id, pod.value); }
  } else { fmt::print("Failed to read PODType objects from file.\n"); }
}

void write_read_several_non_pod_types()
{
  ...
  for (const auto &p : loaded_persons) { fmt::println("Name: {}, Age: {}", p.name, p.age); }
}