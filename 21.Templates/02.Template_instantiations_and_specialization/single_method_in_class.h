#pragma once
/*
    Specializing the get_max function.
*/
...
namespace single_method_in_class {
...
public:
  ...
  T get_max() const
  {
    ...
    for (size_t i{ 0 }; i < m_size; ++i) { if (m_items[i] > m_items[max_index]) { max_index = i; } }
...
//Free operators
template<typename T, typename V> BoxContainer<T, V> operator +(const BoxContainer<T, V> &left,
  const BoxContainer<T, V> &right);
...
const char *BoxContainer<const char *>::get_max() const
{
  ...
  for (size_t i{}; i < m_size; ++i) { if ((strcmp(m_items[i], m_items[max_index])) > 0) { max_index = i; } }
