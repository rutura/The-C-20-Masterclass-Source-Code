...
std::string format_point(const Point& p) {
	return fmt::format("{}", p);
}

template <typename Container>
concept PointContainer = requires(Container c) {
	typename Container::value_type; // Container must have a value_type member
	{ *std::begin(c) } -> std::same_as<Point&>; // Dereferencing must yield a Point&
	{ std::begin(c) != std::end(c) }; // Container must be iterable
};

template <PointContainer Container>
std::string join_points(const Container& cont, const std::string& separator) {
	std::string result;
