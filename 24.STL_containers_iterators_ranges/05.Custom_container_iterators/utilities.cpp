...
#include "custom_iterators_01.h"
#include "custom_iterators_04.h"
#include "custom_iterators_05.h"
#include "custom_iterators_06.h"
#include "custom_iterators_07.h"
#include "custom_iterators_08.h"
#include "custom_iterators_09.h"
...
void custom_iterators_01_demo()
{
  ...
  //find algorithm
  if (std::ranges::find(box1, 8) != box1.end()) { fmt::println("numbers contains: {}", 8); } else {
...
void custom_iterators_06_demo()
{
  ...
  std::cout << "vi odds : ";
  custom_iterators_06::print(std::views::filter(vi, [](int i) { return (i % 2) != 0; }));
  ...
  custom_iterators_05::BoxContainer<custom_iterators_06::Student> class_room;
  //  {{"Mike",12},{"John",17},{"Drake",14},{"Mary",16}};
  ...
  for (auto &s : class_room) { std::cout << "   " << s << std::endl; }
  ...
  custom_iterators_06::print(std::views::take_while(class_room,
    [](const custom_iterators_06::Student &s) { return (s.m_age < 15); }));
...
void custom_iterators_08_demo()
{
  ...
  for (auto i : std::views::take(vi, 3)) { std::cout << i << " "; }
  std::cout << std::endl;
...
void custom_iterators_09_demo()
{
  ...
  for (auto i : greeting) { std::cout << i << " "; }
  std::cout << std::endl;


  std::cout << "taking only 2 : ";
  for (auto i : greeting | std::views::take(2)) { std::cout << i << " "; }
  std::cout << std::endl;
}