#include "utilities.h"
#include "poly_func_and_destructors.h"
...
void do_something_with_animal_ptr(dynamic_casts::Animal *animal)
{
  ...
  if (feline_ptr) { feline_ptr->do_some_feline_thingy(); } else {
...
void do_something_with_animal_ref(dynamic_casts::Animal &animal)
{
  ...
  if (feline_ptr_1) { feline_ptr_1->do_some_feline_thingy(); } else {
...
void dynamic_casts_demo()
{
  ...
  if (feline_ptr_1) { feline_ptr_1->do_some_dog_thingy(); } else {
...
void typeid_operator_demo()
{
  ...
  if (typeid(22.2f) == typeid(float)) { fmt::println("22.2f is a float"); } else { fmt::println("22.2f is not float"); }
