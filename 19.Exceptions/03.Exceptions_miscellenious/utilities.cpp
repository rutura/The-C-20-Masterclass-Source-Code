...
#include "exceptions_in_destructors.h"
#include "noexcept_specifier.h"
#include "rethrowing_exceptions.h"
...
void rethrowing_exceptions_demo()
{

  for (size_t i{ 0 }; i < 5; ++i) {
    try {
      try { rethrowing_exceptions::do_something(i); } catch (rethrowing_exceptions::SomethingIsWrong &ex_inner) {
...
void ellipsis_catch_all_demo()
{
  try {
    for (size_t i{}; i < 5; ++i) {
      try { ellipsis_catch_all::some_function(i); } catch (int ex) { fmt::println("int handler- Cought an integer"); }
      catch (...) {
        fmt::println("Inner... handler , Cought some exception");
        throw;
      }
    }
  } catch (const std::string &ex) { fmt::println("Cought some string exception"); } catch (...) {
    fmt::println("Outer ...handler cought some other exception");
  }
}

void noexcept_specifier_demo()
{
  ...
  try { noexcept_specifier::some_function(); } catch (int ex) { fmt::println("Catching rethrown exception in main"); }
...
void exceptions_in_destructors_demo()
{
  try { exceptions_in_destructors::Item item; } catch (int ex) { fmt::println("main() : Catching int exception"); }
