...
inline generator<int> generate_numbers()
{
...
inline generator<int> infinite_number_stream(int start = 0)
{
  auto value = start;
  for (int i = 0;; ++i) {
    co_yield value;
...
inline generator<int> range(int first, int last) { while (first != last) { co_yield first++; } }
