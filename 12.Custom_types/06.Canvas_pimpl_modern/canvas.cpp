...
namespace modern{
    ...
    //If the implementation of Canvas::Impl in hte modern namespace is stored below this line.
    // We get an error claiming that Canvas::Impl is not defined.
    //By pass the erro by storing it in the module interface file.
    //   Implementation of Canvas::Impl
    //-----------STORE THE IMPLEMETNATION OF Canvas::Impl HERE-----------------//
    ...
    void Canvas::Impl::print() const {
        for (std::size_t x = 0; x < m_width; ++x) {
            for (std::size_t y = 0; y < m_height; ++y) {
                const Pixel& pixel = m_pixels[x * m_width + y];
                auto color = pixel.get_color();
                fmt::print("#{0:06X} ", color);
            }
            fmt::print("\n");
