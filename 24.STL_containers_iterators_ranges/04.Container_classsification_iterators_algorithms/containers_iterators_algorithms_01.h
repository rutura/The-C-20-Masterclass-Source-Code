...
*/
template<std::ranges::input_range Container> void print_container(const Container &container)
{
  for (const auto &elem : container) { fmt::print("{} ", elem); }
  fmt::print("\n");
}
...
inline void use_vector()
{
  ...
  if (it != std::end(vec)) { fmt::println("Found: {}", *it); } else { fmt::println("Not Found"); }
