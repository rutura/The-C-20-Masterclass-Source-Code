...
#include "nested_try_blocks.h"
#include "try_catch_blocks.h"
...
void try_catch_blocks_demo()
{
  ...
  } catch (int ex) { fmt::println("Something went wrong. Exception thrown : {}", ex); }
  fmt::println("Done!");
...
void exceptions_diff_levels_demo()
{

  //exceptions_diff_levels::f1();

  try { exceptions_diff_levels::f1(); } catch (int ex) { fmt::println("Handling execution the closest main()"); }

  fmt::println("END.");

}

void multiple_handlers_for_exception_demo()
{

  try { multiple_handlers_for_exception::f1(); } catch (int ex) { fmt::println("Handling execution in main()"); }
  fmt::println("main() finishing up");
}


void nested_try_blocks_demo()
{
  try { nested_try_blocks::some_function(); } catch (std::string &ex) {
