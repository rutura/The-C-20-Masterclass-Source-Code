...
void Person::serialize(std::ofstream &ofs) const
{
...
void Person::deserialize(std::ifstream &ifs)
{
  size_t name_size;
...
void write_persons_to_file(const std::vector<Person> &persons, const std::filesystem::path &file_path)
{
  ...
  for (const auto &p : persons) { p.serialize(ofs); }
...
std::vector<Person> read_persons_from_file(const std::filesystem::path &file_path)
{
  ...
  for (auto &p : persons) { p.deserialize(ifs); }
