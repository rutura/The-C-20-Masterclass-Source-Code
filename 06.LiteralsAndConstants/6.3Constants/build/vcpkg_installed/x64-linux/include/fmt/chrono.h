...
void write_floating_seconds(memory_buffer &buf, Duration duration, int num_fractional_digits = -1)
{
  ...
  fmt::format_to(std::back_inserter(buf),
    FMT_STRING("{:.{}f}"),
    std::fmod(
      val * static_cast<rep>(Duration::period::num) / static_cast<rep>(Duration::period::den), static_cast<rep>(60)),
...
template<typename T, FMT_ENABLE_IF(std::is_integral<T>::value &&has_isfinite<T>::value)> inline auto isfinite(T) -> bool
{
  return true;
}
